...
export function Pane(props) {
    ...
    useLayoutEffect(() => {
        ...
        if (state.split == 'vertical') {
            ...
            console.log('minSize', state.minSize, 'maxSize', state.maxSize);
            if (state.minSize > 0) {
                patchedStyle.minWidth = state.minSize;
            }
            if (state.maxSize > 0) {
                patchedStyle.maxWidth = state.maxSize;
            }
            if (state.maxSize < 0) {
            ...
            console.log('minSize', state.minSize, 'maxSize', state.maxSize);
            if (state.minSize > 0) {
                patchedStyle.minHeight = state.minSize;
            }
            if (state.maxSize > 0) {
                patchedStyle.maxHeight = state.maxSize;
            }
            if (state.maxSize < 0) {
